...
// Stays entirely within the chosen artist. Pulls their whole known catalogue and
// blends recognition with discovery: the opening deal leads with the artist's
// best-known tracks (so a fan always sees something familiar and doesn't bounce),
// and later rounds lean into deeper cuts. When the catalogue is genuinely
// exhausted it returns 0 candidates (the UI shows an "explored everything" end
// state) — it never wanders to other artists.
export async function recommendDeepDive(artist, profile, excludeIds = new Set()) {
  ...
  // Two tiers from the popularity-ranked catalogue: recognisable "hits" at the
  // top, deep cuts below. We no longer drop the hits — a deep dive that shows
  // only unfamiliar songs gives the user nothing to latch onto, so they skip out
  // early. Each round blends the two; tiers fall back to each other so we always
  // try to fill three.
  const hitCount = Math.max(1, Math.floor(deduped.length * 0.25));
  const hits = deduped.slice(0, hitCount);
  const deep = deduped.slice(hitCount);

  const chosen = new Set();
  function pickOne(tiers) {
    for (const tier of tiers) {
      for (const t of tier) {
        const id = songId(t.artist || artist, t.track);
        if (excludeIds.has(id) || knownSet.has(id) || chosen.has(id)) continue;
        chosen.add(id);
        return t;
      }
    }
  ...
  // First deal hooks with two known tracks + one deep cut; later rounds flip to
  // one familiar + two deep cuts so discovery stays the point.
  const firstRound = excludeIds.size === 0;
  const slots = firstRound
    ? [[hits, deep], [hits, deep], [deep, hits]]
    : [[hits, deep], [deep, hits], [deep, hits]];

  const picks = [];
  for (const tiers of slots) {
    const t = pickOne(tiers);
    if (!t) continue;
    picks.push({
      track: t.track,
  ...
  const enriched = await Promise.all(
    picks.map(async (c) => {
      ...
      return {
        ...e,
        why: c._fromHits ? `One of ${artist}'s best known.` : `A deeper cut from ${artist}.`,
      };
    })
  );
  return { candidates: enriched, fallbackLog: log };
